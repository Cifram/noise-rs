...
fn bench_open_simplex4(c: &mut Criterion) {
    ...
    c.bench_function("open simplex 4d", |b| {
        b.iter(|| open_simplex_4d(black_box(Vector4::new(42.0_f64, 37.0, 26.0, 128.0)), &hasher))
    });
}
...
fn bench_open_simplex3_64x64(c: &mut Criterion) {
    ...
    c.bench_function("open simplex 3d (64x64)", |b| {
        b.iter(|| {
            for y in 0i8..64 {
                for x in 0i8..64 {
                    open_simplex_3d(black_box(Vector3::new(x as f64, y as f64, x as f64)), &hasher);
                }
            }
        })
    });
...
fn bench_open_simplex4_64x64(c: &mut Criterion) {
    ...
    c.bench_function("open simplex 4d (64x64)", |b| {
        b.iter(|| {
            for y in 0i8..64 {
                for x in 0i8..64 {
                    open_simplex_4d(black_box(Vector4::new(x as f64, y as f64, x as f64, y as f64)), &hasher);
                }
            }
        })
    });

...
fn bench_super_simplex2(c: &mut Criterion) {
    ...
    c.bench_function("super simplex 2d", |b| {
        b.iter(|| black_box(super_simplex_2d(Vector2::new(42.0_f64, 37.0), &hasher)))
    });
}
...
fn bench_super_simplex3(c: &mut Criterion) {
    ...
    c.bench_function("super simplex 3d", |b| {
        b.iter(|| black_box(super_simplex_3d(Vector3::new(42.0_f64, 37.0, 26.0), &hasher)))
    });
}
...
fn bench_super_simplex2_64x64(c: &mut Criterion) {
    ...
    c.bench_function("super simplex 2d (64x64)", |b| {
        b.iter(|| {
            for y in 0i8..64 {
                for x in 0i8..64 {
                    black_box(super_simplex_2d(Vector2::new(x as f64, y as f64), &hasher));
                }
            }
        })
    });
...
fn bench_super_simplex3_64x64(c: &mut Criterion) {
    ...
    c.bench_function("super simplex 3d (64x64)", |b| {
        b.iter(|| {
            for y in 0i8..64 {
                for x in 0i8..64 {
                    black_box(super_simplex_3d(Vector3::new(x as f64, y as f64, x as f64), &hasher));
                }
            }
        })
    });

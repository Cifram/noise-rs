...
use noise::{
    core::worley::{worley_4d_value, worley_4d_range, worley_4d_range_sqr, worley_4d_border},
...
fn bench_worley4d_border(c: &mut Criterion) {
    let hasher = PermutationTable::new(0);
    c.bench_function("worley 4d border", |b| {
        b.iter(|| worley_4d_border(black_box(Vector4::new(42.0f64, 37.0, 26.0, 128.0)), &hasher))
    });
}
...
fn bench_worley4d_border_64x64(c: &mut Criterion) {
    let hasher = PermutationTable::new(0);
    c.bench_function("worley 4d border (64x64)", |b| {
        b.iter(|| {
            for y in 0i8..64 {
                for x in 0i8..64 {
                    black_box(worley_4d_border(Vector4::new(x as f64, y as f64, x as f64, y as f64), &hasher));
                }
            }
        })
    });
